...
| Світла | Темна | Тепла |
|--------|-------|-------|
| ![Дашборд Світла](screenshots/dashboard-light.png) | ![Дашборд Темна](screenshots/dashboard-dark.png) | ![Дашборд Тепла](screenshots/dashboard-warm.png) |
| ![Вхід Світла](screenshots/login-light.png) | ![Вхід Темна](screenshots/login-dark.png) | ![Вхід Тепла](screenshots/login-warm.png) |
| ![404 Світла](screenshots/not-found-light.png) | ![404 Темна](screenshots/not-found-dark.png) | ![404 Тепла](screenshots/not-found-warm.png) |

| Головна сторінка | Проміжна сторінка |
|------------------|-------------------|
| ![Головна](screenshots/index.png) | ![Проміжна](screenshots/interstitial.png) |

## Технологічний Стек

- **Мова**: Rust (асинхронний з Tokio)
- **Веб-фреймворк**: Axum
- **Кеш**: Redis-compatible (Redis, Dragonfly, Valkey, KeyDB)
- **База даних**: PostgreSQL (змінний шар зберігання)
- **Метрики**: Prometheus + metrics-rs
- **Хешування паролів**: Argon2

> **Примітка**: PostgreSQL використовується як бекенд зберігання за замовчуванням. Шар зберігання абстрагований і може бути замінений будь-яким джерелом даних. Наразі в активній розробці.
...
| `GET /r/{hashid}` | Ні | Перенаправлення з проміжною сторінкою |
| `GET /health` | Ні | Перевірка стану |
| `GET /metrics` | Basic | Метрики Prometheus |
| `GET /admin` | Сесія | Вхід до панелі адміністратора |
| `GET /admin/dashboard` | Сесія | Панель адміністратора |

## Панель Адміністратора

Сервіс включає опціональну панель адміністратора для моніторингу метрик у реальному часі.
...
# Введіть пароль за запитом, або:
cargo run --bin hash_password -- "ваш-пароль"
```

2. **Додайте до config.yaml:**
...
Перейдіть на `http://localhost:8080/admin` та увійдіть зі своїми обліковими даними.

### Можливості

- Графіки RPS та затримки в реальному часі
- Системні метрики (CPU, пам'ять, час роботи)
- Моніторинг коефіцієнта влучань кешу
- Список останніх перенаправлень
