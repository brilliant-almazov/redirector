...
- 📤 **イベント分析** - RabbitMQへのオプションイベントパブリッシングとPostgreSQLコンシューマー
...
- **メッセージキュー**: RabbitMQ（オプション、イベント分析用）
- **パスワードハッシュ**: Argon2
...
リダイレクト分析のためのオプションのイベントパブリッシングパイプライン。有効にすると、各リダイレクトイベントがRabbitMQに公開され、別のバイナリによってPostgreSQLに書き込まれます。

> **完全なドキュメント**: [EVENT_ANALYTICS.md](EVENT_ANALYTICS.md)

### 機能

- **Fire-and-forget パブリッシング** — リダイレクトのレイテンシはキューの可用性に影響されない
- **バッチ処理** — イベントをサイズ（100件）または時間（1秒）でグループ化
- **User-Agent解析** — wootheeによるブラウザ、バージョン、OS、デバイスタイプ
- **GeoIPエンリッチメント** — IPから国と都市（MaxMind mmdb、ホットリロード対応）
- **参照の重複排除** — refererとuser agentsのMD5ベースの重複排除
- **月次パーティショニング** — `redirect_events`の自動パーティション作成
...
イベント処理パイプラインは専用のPrometheusメトリクスを公開します：

- `event_publisher_published_total` — 公開されたイベント数
- `event_publisher_queue_size` — キュー内のイベント数
- `event_consumer_processed_total` — 処理済みイベント数
- `event_consumer_batch_size` — バッチサイズ
- `event_consumer_processing_duration_seconds` — 処理時間
- `geoip_cache_hits_total` — GeoIPキャッシュヒット数
- `geoip_cache_misses_total` — GeoIPキャッシュミス数
...
Redirect Handler (async)
  ├─ Publisher → RabbitMQ (Fire-and-forget)
  └─ Interstitial Page → Redirect

Event Consumer
  ├─ RabbitMQ (batch poll)
  ├─ Enrich (User-Agent, GeoIP)
  ├─ Deduplicate (MD5)
  └─ PostgreSQL (bulk insert with partitioning)
```
